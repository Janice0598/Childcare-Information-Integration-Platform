...
async function loadMyReviews() {
  ...
  document.getElementById("user-name-display").innerText =
    sessionStorage.getItem("loggedInUserName") || "家長";
  ...
  try {
    ...
    listContainer.innerHTML = reviews
      .map((r) => {
        const centerName = r.childcare_center
          ? r.childcare_center.name
          : "未知機構";
        const date = r.created_at ? r.created_at.slice(0, 10) : "-";
        return `
            <div class="result-card" style="flex-direction:column;padding:20px;background:#fff;" id="review-card-${
              r.review_id
            }">
