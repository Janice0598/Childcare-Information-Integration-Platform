...
async function loadRecommendations() {
    const preferences = sessionStorage.getItem('parentPreferences');
    let pref = {};
    if (preferences) {
        try { pref = JSON.parse(preferences); } catch(e) { console.warn(e); }
    }
    ...
    try {
        ...
        if (pref.district && pref.district !== '') query = query.eq('district', pref.district);
        if (pref.category && pref.category !== '') query = query.eq('category', pref.category);
        if (pref.extended && pref.extended !== '') {
            query = query.eq('extended_care', pref.extended === 'true');
        }
        ...
        if (!centers || centers.length === 0) {
            recDiv.innerHTML = '<div class="text-line" style="text-align:center;">沒有符合偏好的機構。</div>';
            return;
        }
